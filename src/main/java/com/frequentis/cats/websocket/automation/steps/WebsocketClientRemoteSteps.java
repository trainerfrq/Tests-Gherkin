...
public class WebsocketClientRemoteSteps extends WebsocketAutomationSteps {
    ...
    public void openNamedWebSocketConnections(final List<ProfileToWebSocketConfigurationReference> namedProfileToWebSocketConfigReferences) {
        RemoteStep remStep = remoteStep("Parsing Datatables paralelly");
        ...
        if (namedProfileToWebSocketConfigReferences.size() == 0) {
            localStep("looking into tables for parsing")
                    .details(ExecutionDetails.create("Parsing Table")
                            .expected("Table contains at least one entry")
                            .received("NO ENTRY FOUND " + namedProfileToWebSocketConfigReferences)
                            .success(false));
        }

        int i = 1;
        for (ProfileToWebSocketConfigurationReference reference : namedProfileToWebSocketConfigReferences) {
            ...
            localStep("Parsing Table")
                    .details(ExecutionDetails.create("Parse data table. Entry " + i++ + " of " + namedProfileToWebSocketConfigReferences.size())
                            .expected("ExamplesTable can be parsed")
                            .receivedData(reference.getKey(), reference)
                            .success(true));
            final ClientEndpointConfiguration config = getStoryListData(reference.getWebSocketConfigurationName(), ClientEndpointConfiguration.class);

            ArrayList<String> endpointName = new ArrayList<String>();
            endpointName.add(reference.getKey());

            execution =
                    remStep.scriptOn(
                            profileScriptResolver()
                                    .map(OpenWebSocketClientConnection.class, BookableProfileName.websocket),
    ...
    public void startRecordingOnNamedWebSocket(final String namedWebSocket, final Integer bufferSize) {
        ProfileToWebSocketConfigurationReference reference = getStoryListData(namedWebSocket, ProfileToWebSocketConfigurationReference.class);

        evaluate(
                remoteStep("Start recording on named WebSocket: " + namedWebSocket).scriptOn(
                        profileScriptResolver()
                                .map(
                                        StartClientConnectionRecording.class, BookableProfileName.websocket),
                        requireProfile(reference.getProfileName()))
    ...
    public void sendMessageAsIsOverNamedWebSocketAndNameMessage(final String namedWebSocket, final String namedMessage, final String namedTimeStamp) {
        ProfileToWebSocketConfigurationReference reference = getStoryListData(namedWebSocket, ProfileToWebSocketConfigurationReference.class);
        String messageToSend = assertStoryListData(namedMessage, String.class);
        RemoteStepResult remoteStepResult = evaluate(
                remoteStep("Sending message with using " + namedWebSocket)
                        .scriptOn(profileScriptResolver()
                                        .map(
                                                SendTextMessageAsIs.class, BookableProfileName.websocket),
                                requireProfile(reference.getProfileName()))
    ...
    public void sendMessagesAsIsOverNamedWebSocket(final String namedWebSocket, final List<CatsCustomParameterBase> namedMessageList) {
        ProfileToWebSocketConfigurationReference reference = getStoryListData(namedWebSocket, ProfileToWebSocketConfigurationReference.class);
        List<String> messageToSendList= new ArrayList<>();
        for (CatsCustomParameterBase parameter : namedMessageList) {
            String namedMessageKey=parameter.getKey();
            String messageToSend=assertStoryListData(namedMessageKey, String.class);
        ...
        RemoteStepResult remoteStepResult= evaluate(
                remoteStep("Sending message with using "+ namedWebSocket)
                        .scriptOn(profileScriptResolver()
                                        .map(
                                                SendTextMessageAsIsParallel.class, BookableProfileName.websocket),
                                requireProfile(reference.getProfileName()))
                        .input(SendTextMessageAsIsParallel.IPARAM_ENDPOINTNAME, namedWebSocket)
                        .input(SendTextMessageAsIsParallel.IPARAM_MESSAGETOSEND, (Serializable) messageToSendList)
        );
    }
    ...
    public void receiveMessageAndValidate(final String namedWebSocket, final String expectedMessage, final String namedTimeStamp){
        ProfileToWebSocketConfigurationReference reference = getStoryListData(namedWebSocket, ProfileToWebSocketConfigurationReference.class);
        ...
        RemoteStepResult remoteStepResult= evaluate(
                remoteStep("Sending message with using "+ namedWebSocket)
                        .scriptOn(profileScriptResolver()
                                        .map(
                                                ReceiveMessageAsIs.class, BookableProfileName.websocket),
                                requireProfile(reference.getProfileName()))
                        .input(ReceiveMessageAsIs.IPARAM_ENDPOINTNAME,namedWebSocket)
        );
        String receviedMessage= (String) remoteStepResult.getOutput(ReceiveMessageAsIs.OPARAM_RECEIVEDMESSAGE);
        String expectedMessageString=assertStoryListData(expectedMessage, String.class);
        if(receviedMessage!=null) {
            LocalStep checkReceivedMessage = localStep("Check received Message");
            checkReceivedMessage.details(
                    ExecutionDetails
                            .create("Check received Message")
                            .received(receviedMessage)
                            .expected(expectedMessageString)
