...
//    /**
//     * The agent used to run the web socket client.
//     */
//    protected transient AgentBase agent;
//
//    /**
//     * Default constructor.
//     */
//    WebsocketScriptTemplate() {
//        super(ScriptLanguage.GROOVY, false);
//    }
//
//    /**
//     * Gets the AgentMain instance.
//     * @return
//     */
//    @Override
//    boolean preSteps() {
//        super.preSteps();
//        this.agent = AgentBase.getAgent();
...
//    }
     ...
     */
    protected ClientEndpoint createWebSocketEndpoint(
            final String endpointKey, final ClientEndpointConfiguration configuration) {
        final ClientEndpointManager manager = getWebSocketEndpointManager();
        ...
        if (endpoint != null) {
            try {
                if (!endpoint.isConnected()) {
                    LOGGER.error("WebSocket Client Endpoint NOT Connected: [Key:'" + endpointKey + "', URI:'" + endpoint.getUri() + "']");
                    return false;
                }
            ...
            LOGGER.error("Error sending text message: [" + message + "]. Endpoint with key [" + endpointKey + "] couldn't be found.");
        }
        return false;
    }
     ...
     * @return The message or null if not received within given wait timeout.
     */
    protected List<TextMessage> receiveText(
            final String endpointKey, final String bufferKey, final int nMessageCount, final long nWait) {
        List<TextMessage> polledMessages = null;
        final MessageBuffer buffer = getMessageBuffer(endpointKey, bufferKey);
        if (buffer != null) {
            try {
                WaitCondition condition = new WaitCondition("Wait for certain amount of messages") {
                    @Override
                    public boolean test() {
     ...
     * @return The message or null if not received within given wait timeout.
     */
    protected List<BinaryMessage> receiveBinary(
            final String endpointKey, final String bufferKey, final int nMessageCount, final long nWait) {
     ...
     */
    protected boolean startPing(final long pingInterval, final TimeUnit timeUnit, TextMessage message) {
    ...
    protected boolean startPing(final String endpointKey, final long pingInterval, final TimeUnit pingIntervalTimeUnit,
                                TextMessage message, final long delayInterval, final TimeUnit delayIntervalUnit) {
        final ClientEndpoint endpoint = getWebSocketEndpoint(endpointKey);
        if (endpoint != null) {
            try {
                LOGGER.info("Starting ping scheduler with ping interval: " + pingInterval);
                final long nanoDelayInterval = delayInterval > 0 && delayIntervalUnit != null ?
                        TimeUnit.NANOSECONDS.convert(delayInterval, delayIntervalUnit):
                        delayInterval;
                return endpoint.startPing(pingInterval, pingIntervalTimeUnit, message,
                        nanoDelayInterval) != null;
            } catch (Exception ex) {
                LOGGER.error("Error starting ping scheduler with message: [" + message.toString() + "] and interval: [" + pingInterval + "]", ex);
            }
        } else {
            LOGGER.error("Error starting ping scheduler. Couldn't get websocket endpoint with endpoint key: '" + endpointKey + "'.");
        }
        return false;
    }
    ...
    protected PingSender getPing(final String endpointKey) {
        ...
        if (endpoint != null) {
            ...
            LOGGER.error("Error getting ping scheduler. Couldn't get websocket endpoint with endpoint key: '" + endpointKey + "'.");
        }
        return pingSender;
    }
