...
public class CallUISteps extends AutomationSteps
{
   ...
   {
      final DAKey daKey = retrieveDaKey( profileName, target );
   ...
   {
      final FunctionKey functionKey = retrieveFunctionKey( type );
   ...
   public void verifyReadyToBeUsedDAState( final String profileName, final String target )
   {
      ...
      evaluate(
            remoteStep( "Check application status" )
                  .scriptOn( profileScriptResolver().map( VerifyDAButtonUsageReady.class, BookableProfileName.javafx ),
                        assertProfile( profileName ) )
   ...
   {
      final FunctionKey key = retrieveFunctionKey( target );
