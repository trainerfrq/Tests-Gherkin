...
public class PhoneBookUISteps extends AutomationSteps {
    ...
    public void verifyExistanceForwardButtonState(final String profileName, final String state) {
        evaluate(remoteStep("Verify forward button is " + state)
                         .scriptOn(
                                 profileScriptResolver().map(VerifyPhoneBookForwardButtonIfVisible.class, BookableProfileName.javafx),
                                 assertProfile(profileName))
    ...
    public void assignValuesForCallRouteSelectors(final String profileName, final String givenMission, final String path) throws IOException {
        ...
        final String response =
                (String) remoteStepResult.getOutput(VerifyCallRouteSelectorList.OPARAM_RECEIVED_CALL_ROUTE_SELECTORS);
