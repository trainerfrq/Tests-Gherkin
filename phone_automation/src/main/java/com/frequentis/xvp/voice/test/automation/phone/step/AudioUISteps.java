...
import scripts.cats.hmi.actions.ClickOnIdleWarningPopupButton;
...
public class AudioUISteps extends AutomationSteps
{
   ...
   @Then("$profileName verifies that warning popup is visible")
   public void verifyWarningPopupVisibility( final String profileName )
   {
   ...
   public void openPopup( final String profileName, final String panelName )
   {
   ...
   public void clickunattendedButton( final String profileName, final String buttonName )
   {
      evaluate( remoteStep( "Choose option " + buttonName + " from idle warning popup" ).scriptOn(
            profileScriptResolver().map( ClickOnIdlePopupButton.class, BookableProfileName.javafx ),
            assertProfile( profileName ) )
            .input( ClickOnIdleWarningPopupButton.IPARAM_BUTTON_NAME, buttonName ) );
   }
}
