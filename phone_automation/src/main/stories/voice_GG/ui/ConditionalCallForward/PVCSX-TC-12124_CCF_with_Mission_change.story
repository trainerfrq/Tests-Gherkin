...
OP1 has TWR role assigned.
@TEST_CASE_PASS_FAIL_CRITERIA: This test is passed if the call to an operator (which has as active role, a role matching the call destination of the configured conditional call forward rule) is forwarded according to the configured conditional call forward rule, even if the operator changed its mission
...
Scenario: 4. OP3 accepts the call
...
Scenario: 6. OP2 establishes a call to OP1
Meta:
...
Scenario: 6.1 OP1 receives the call
Then HMI OP1 has the call queue item OP2-OP1 in state inc_initiated

Scenario: 6.2 Verifying call queue section
...
Scenario: 7. OP1 changes its mission to TWR
Meta:
