...
Scenario: Third client retrieves phone data
When WS3 loads phone data for role roleId3 and names secondCallSource and secondCallTarget from the entry number 1

Scenario: Third client establishes an outgoing call
When WS3 establishes an outgoing phone call using source secondCallSource ang target secondCallTarget and names outgoingPhoneCallId2
And waiting for 1 seconds
...
Scenario: First client receives the incoming call and confirms it
When WS1 receives call incoming indication on message buffer named CallIncomingIndicationBuffer1 with callSource and callTarget and names incomingPhoneCallId2
...
Scenario: First client answers the incoming call
When WS1 answers the incoming phone call with the callId incomingPhoneCallId2
And waiting for 1 seconds
Then WS1 receives call status indication on message buffer named CallStatusIndicationBuffer1 with callId incomingPhoneCallId2 and status connected
...
Scenario: First client puts the call on hold
When WS1 puts the phone call with the callId incomingPhoneCallId2 on hold
And waiting for 2 seconds

Scenario: Verify call is on hold
Then WS1 receives call status indication on message buffer named CallStatusIndicationBuffer1 with callId incomingPhoneCallId2 and status hold
Then WS3 receives call status indication on message buffer named CallStatusIndicationBuffer3 with callId outgoingPhoneCallId2 and status held
...
Then WS1 does NOT receive call status indication verifying all the messages on message buffer named CallStatusIndicationBuffer1 with callId incomingPhoneCallId2 and status connected
Then WS3 does NOT receive call status indication verifying all the messages on message buffer named CallStatusIndicationBuffer3 with callId outgoingPhoneCallId2 and status connected
