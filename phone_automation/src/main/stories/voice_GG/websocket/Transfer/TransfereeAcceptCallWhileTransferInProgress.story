Narrative:
As a transferee operator having on hold the initial call which is getting transferred
I want to accept another call
So I can verify that the initial call will not be auto-terminated
...
| calledTarget | sip:222222 |

Scenario: Transferee receives the incoming call and confirms it
When WS2 receives call incoming indication on message buffer named CallIncomingIndicationBuffer2 with calledSource and calledTarget and names phoneIncomingCallId
And WS2 confirms incoming phone call with callId phoneIncomingCallId
Then SipContact DialogState is EARLY within 100 ms

Scenario: Transferee puts on hold initial call
When WS2 puts the phone call with the callId incomingPhoneCallId1 on hold
And waiting for 3 seconds

Scenario: Initial call is put on hold
Then WS1 receives call status indication with call conditional flag xfr on message buffer named CallStatusIndicationBuffer1 with callId outgoingPhoneCallId1 and status two_way_hold
Then WS2 receives call status indication on message buffer named CallStatusIndicationBuffer2 with callId incomingPhoneCallId1 and status two_way_hold

Scenario: Transferee answers the incoming call
When WS2 answers the incoming phone call with the callId phoneIncomingCallId
Then WS2 receives call status indication on message buffer named CallStatusIndicationBuffer2 with callId phoneIncomingCallId and status connected
...
And waiting for 2 seconds
Then WS2 receives call status indication with terminated status on message buffer named CallStatusIndicationBuffer2 with callId phoneIncomingCallId and terminationDetails normal

Scenario: Transferee retrieves from hold consultation call
When WS2 retrieves the on hold phone call with the callId incomingPhoneCallId1
And waiting for 3 seconds
